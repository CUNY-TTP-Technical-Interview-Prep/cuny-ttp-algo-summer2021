...
/**input:
   arr: unsorted array
   output:
   return all unique triplet that equal zero
   
   ---
   arr(sort) to make it ascending order
   initiate a for loop
   left and right variable
   left should be i+1 and right should be array.length -1
   arr[i] !== arr[i-1]
   check to see if no duplicates 
   if not then left is equal to i + 1;
   right == arr.length-1;
